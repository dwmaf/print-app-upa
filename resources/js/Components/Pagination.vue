<template>
    <div v-if="links.length > 3"
        class="inline-flex items-stretch border border-gray-300 divide-x-2 divide-gray-300 rounded-lg overflow-hidden">
        <template v-for="(link, key) in links" :key="key">
            <template v-if="key === 0 && link.url">
                <Link :href="link.url ?? '#'" :disabled="!link.url"
                    class="px-2 py-2 text-sm font-bold flex items-center text-gray-700 justify-center"
                    :class="{
            ...
            <template v-else-if="key !== 0 && key !== links.length - 1">
                ...
                <Link v-else class="px-3 py-2 text-sm font-bold hover:bg-gray-100"
                    :class="[link.active ? 'text-indigo-700' : 'text-gray-700']"
    ...
    <div v-if="links.length < 3" class="flex justify-between w-full">
        <Link v-if="links[0].url" :href="links[0].url"
            class="px-4 py-2 text-sm font-bold flex items-center text-gray-700 justify-center border border-gray-300 rounded-lg hover:bg-gray-100">
            <span>Previous</span>
        </Link>
        <div v-else></div>
